...
export const startInteractiveObjects = (renderer:WebGlRenderer) => ({basicPath, gltfPath}:{basicPath: string, gltfPath: string}) => {
        ...
        return (createSkybox(renderer) as Future<any, (camera:Camera) => (frameTs:number) => void>)
            .map(render => ([render]))
            .chain(xs => 
                _getBridge (renderer) (gltfPath + getModel("BOOM_BOX_AXES").url)
                .map(render => xs.concat([render]))
            )
            /*.chain(xs => 
                _getBridge (renderer) (gltfPath + getModel("DAMAGED_HELMET_BINARY").url)
                .map(render => xs.concat([render]))
            )
            */
            .map(renderers => {
                ...
                return [
                    (frameTs:number) => {
                        ...
                        renderers.forEach(render => render (camera) (frameTs))
                    },
                    () => {
                        controls.disable(); 
                    }
